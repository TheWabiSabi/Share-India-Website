...
import { FaChevronRight, FaShieldAlt, FaFileContract, FaPhoneAlt, FaEnvelope, FaHeadset } from 'react-icons/fa';
...
export interface CorporateInsuranceDetails {
  ...
  coverage: { desciption: string; coverages: Array<{ title: string; icon: React.ReactNode; body: string; bullets: string[] }> };
...
const CorporatePage = ({ details, questions }: { details: CorporateInsuranceDetails; questions?: Question[] }) => (
  <div className="min-h-screen bg-white text-gray-900">

    {/* ── Hero ── */}
    <section className="relative isolate flex min-h-[80dvh] items-end overflow-hidden pt-16">
      <div className="absolute inset-0">
        <Image src={details.img} alt={details.hero.title} fill priority className="object-cover" sizes="100vw" />
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl text-white">
            <div className="border-si-primary/30 bg-si-primary/15 mb-5 inline-flex items-center gap-2 rounded-full border px-4 py-2 backdrop-blur-sm">
              ...
              <span className="text-sm font-semibold text-blue-100">Corporate Insurance Specialists • IRDAI Licensed</span>
            </div>
            <h1 className="text-4xl font-extrabold leading-tight tracking-tight md:text-5xl">{details.hero.title}</h1>
            <p className="mt-5 text-lg leading-relaxed text-gray-200">{details.hero.description}</p>
            <div className="mt-7 flex flex-wrap gap-3">
              <a href="/premium-estimator" className="btn-primary inline-flex items-center gap-2 rounded-xl px-6 py-3 font-semibold">
                <FaShieldAlt /> Get Coverage Proposal
              </a>
              <a href="/contact" className="btn-ghost inline-flex items-center gap-2 rounded-xl border-white/30 bg-white/10 px-6 py-3 font-semibold text-white hover:bg-white/20">
    ...
    {/* ── Explanation ── */}
    <section className="border-b border-gray-100 bg-[#f0f7ff] py-16 lg:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-10">
          <h2 className="text-si-ink text-3xl font-bold md:text-4xl">{details.explaination.title}</h2>
          <p className="text-si-ink/70 mt-4 max-w-3xl text-lg leading-relaxed">{details.explaination.description}</p>
        </div>
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
          <div>
            <h3 className="text-si-ink mb-5 text-xl font-bold">Overview</h3>
            <p className="text-si-ink/70 mb-5 leading-relaxed">{details.explaination.overview.description}</p>
          ...
          <div>
            ...
            <div className="space-y-4">
              {details.explaination.projectTypes.types.map((type, i) => (
                <div key={i} className="flex items-start gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm transition-all hover:border-si-primary/20 hover:shadow-md">
                  <div className="text-si-primary bg-si-primary/8 shrink-0 rounded-xl p-2.5 text-xl">{type.icon}</div>
                  <div>
    ...
    {/* ── Claim Stories ── */}
    <section className="border-b border-gray-100 bg-[#f0f7ff] py-14">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
          <MainCaraousel color="blue" layout={1} topic={details.claim_story.topic} type="claims_story" />
        </div>
      </div>
    </section>
    ...
    <section className="border-b border-gray-100 bg-white py-14">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <h2 className="text-si-ink mb-3 text-2xl font-bold md:text-3xl">Industry Insights & Articles</h2>
        <p className="text-si-ink/70 mb-8">Stay updated with the latest trends, regulations, and best practices</p>
        <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
    ...
    {/* ── Knowledge Quiz ── */}
    {questions && <KnowledgeQuestionnaire questions={questions} industryName={details.knowledge.industryName} />}
        ...
        <div className="border-si-primary/10 from-si-primary/5 overflow-hidden rounded-2xl border bg-gradient-to-br to-white p-8 shadow-sm md:p-10">
          <div className="grid grid-cols-1 items-center gap-6 lg:grid-cols-3">
            <div className="lg:col-span-2">
              <h3 className="text-si-ink text-2xl font-bold">Protect Your Business. Secure Your Future.</h3>
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <Contact label="Advisory Desk" value="Corporate Insurance Specialists" icon={<FaHeadset />} />
          </div>
        </div>
      </div>
    </section>
