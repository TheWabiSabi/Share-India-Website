...
const logos = [
  ...
  { input: 'public/share-india-transparent_header.png', output: 'public/share-india-transparent_header.webp' },
...
async function main() {
  ...
  for (const { input, output } of logos) {
    ...
    try {
      ...
      console.log(`✅ ${path.basename(input)}: ${(originalSize / 1024).toFixed(0)}KB → ${(webpSize / 1024).toFixed(0)}KB (${savingsPercent}% savings)`);
