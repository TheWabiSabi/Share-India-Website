...
const slugify = (s: string) =>
  s.toLowerCase().replace(/ & /g, '-and-').replace(/\s+/g, '-').replace(/[^a-z-]/g, '');
...
export default function Header() {
  ...
  const baseLinkCls = 'transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-si-primary/30';
  ...
  const primaryLinks: NavItem[] = [
    ...
    },
    {
      title: 'Industries',
      href: '/industries',
      children: INDUSTRIES.map((t) => ({ label: t, href: `/industries/${slugify(t)}`, desc: 'Solutions & case studies' })),
    },
    {
      title: 'Corporate Insurance',
      href: '/corporate-insurance',
      children: CORPORATE_INSURANCE.map((t) => ({ label: t, href: `/corporate-insurance/${slugify(t)}`, desc: 'Coverage, FAQs & claims' })),
    },
    {
      title: 'Insights',
      href: '/insights',
      children: [
        { label: 'Claim Stories', href: '/insights/claim-stories', desc: 'Real journeys & learnings' },
    ...
    },
    {
      ...
      children: [
        ...
        { label: 'Awards & Recognition', href: '/about#awards-recognition', desc: 'Milestones & credibility' },
  ...
  return (
    <header
      className={[
        ...
        scrolled
          ? 'border-b border-si-primary/10 bg-white/97 shadow-[0_4px_24px_-4px_rgba(45,169,255,0.12)] backdrop-blur-md'
    ...
    >
            ...
            {primaryLinks.map((link) => {
              ...
              if (!link.children?.length) {
                return (
                  ...
                  >
                    {link.title}
                    {active && <span className="bg-si-primary absolute inset-x-0 -bottom-0.5 h-0.5 rounded-full" />}
                  </Link>
                );
              }
              ...
              return (
                ...
                >
                  <button
                    type="button"
                    onClick={() => setOpenDesktopDropdown((p) => p === link.title ? null : link.title)}
                  ...
                  >
                    <span className="relative">
                      {link.title}
                      <span className={`bg-si-primary absolute -bottom-0.5 left-0 block h-0.5 transition-all duration-200 ${openDesktopDropdown === link.title || active ? 'w-full' : 'w-0 group-hover:w-full'}`} />
                    </span>
                    <FaChevronDown className={`text-[10px] transition-transform duration-200 ${openDesktopDropdown === link.title ? 'rotate-180 text-si-primary' : ''}`} />
                  </button>
                  ...
                  <AnimatePresence>
                    {openDesktopDropdown === link.title && (
                      <motion.div
                        ...
                        className="absolute top-full left-0 z-50 mt-3 w-[40rem] overflow-hidden rounded-2xl border border-si-primary/10 bg-white shadow-[0_20px_48px_-12px_rgba(45,169,255,0.2),0_0_0_1px_rgba(45,169,255,0.06)]"
                        onMouseEnter={cancelClose}
                        onMouseLeave={() => scheduleClose(120)}
                        role="menu"
                      >
                        {/* Top accent bar */}
                        <div className="bg-gradient-to-r from-si-primary via-si-primary-400 to-si-primary-600 h-0.5 w-full" />
                        <div className="mx-auto grid gap-1 p-3 sm:grid-cols-2">
                          {link.children.map((child) => (
                            <Link
                              key={child.label}
                              href={child.href}
                              className="group flex flex-col rounded-xl px-3 py-2.5 transition-all hover:bg-si-primary/5 focus-visible:outline-none"
                              role="menuitem"
                            ...
                            >
                              ...
                              {child.desc && (
                                <span className="text-si-ink/50 mt-0.5 text-[11.5px]">{child.desc}</span>
                              )}
                            </Link>
                          ))}
                        </div>
                        <div className="border-t border-si-primary/8 mx-3 mb-2 px-1 py-2 text-right">
                          <Link
                            href={link.href}
      ...
      >
        <div
          className={`absolute inset-0 bg-si-dark/40 backdrop-blur-sm transition-opacity duration-300 ${mobileMenu ? 'opacity-100' : 'opacity-0'}`}
        ...
        >
          {/* Drawer header */}
          <div className="sticky top-0 z-10 flex items-center justify-between border-b border-si-primary/10 bg-white px-5 py-4">
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5">
            <nav className="flex flex-col gap-1 text-[15px] font-medium text-si-ink">
              {primaryLinks.map((link) => {
                if (!link.children?.length) {
                  return (
                    <Link
                ...
                return (
                  <div key={link.title} className="rounded-xl overflow-hidden">
                    <button
                      type="button"
                      onClick={() => setOpenMobileAccordion((p) => p === link.title ? null : link.title)}
                    ...
                    >
                      <div className="ml-3 flex flex-col gap-0.5 border-l border-si-primary/15 pb-2 pl-3 pt-1">
                        {link.children.map((child) => (
                          <Link
                            key={child.label}
                            href={child.href}
                            onClick={() => setMobileMenu(false)}
                            className="hover:text-si-primary rounded-lg px-2 py-2 text-[13.5px] text-si-ink/80 transition-colors hover:bg-si-primary/5"
                          >
                            {child.label}
                            {child.desc && <span className="block text-[11px] text-si-ink/40">{child.desc}</span>}
                          </Link>
                        ))}
                      </div>
                    </div>
            ...
            <div className="mt-6 flex flex-col gap-2.5 border-t border-si-primary/10 pt-5">
              <Link
                href="/be-a-posp"
                onClick={() => setMobileMenu(false)}
